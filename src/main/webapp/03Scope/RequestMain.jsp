...
/* 
	request영역 : 클라이언트가 요청을 할때마다 새로운 request객체가 
		생성되어 같은 요청을 처리하는데 사용되는 모든 JSP페이지가 영역을
		공유한다. 즉 request영역에 저장된 정보는 현재페이지와 
		forward된 페이지까지 공유된다.
*/
...
<body>
	...
	/* 
		포워드 : 현재 페이지로 들어온 요청을 다음 페이지로 보내는 기능을 말한다.
			...
			만나기 전까지 모든 내용은 버퍼에서 삭제되고, 포워드된 페이지에
			내용만 화면에 출력하는 특징을 가진다. 즉 포워드란 현재페이지로 들어온
			요청을 다른 페이지에서도 공유할 수 있도록 전달하는 것을 말한다.
	*/
