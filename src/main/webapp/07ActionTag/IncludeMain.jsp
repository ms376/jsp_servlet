...
<%-- 	<%@ include file = "<%=outerPath1OuterPage1%>"%> --%>
	<!-- 
		지시어를 통한 인클루드는 포함한 파일의 소스를 그대로 현재 문서에
		포함시킨 후 컴파일을 진행합니다. 따라서 하나의 page라 생각하면됨
		그러므로 newVar1은 정상적으로 출력됨
	 -->
	<p>
		외부파일에 선언한 변수 :
		<%=newVar1%></p>
	...
	<!-- 
		액션태그를 통한 인클루드는 포함된 파일을 먼저 실행(컴파일)한 후
		실행된 결과를 해당 문서에 포함시킨다. 따라서 외부파일에서 선언한
		변수를 현재 페이지에서 사용할 수 없게된다.
	 -->
	<p>
		외부 파일에 선언한 변수 :
<%-- 		<%=newVar2%> --%>
