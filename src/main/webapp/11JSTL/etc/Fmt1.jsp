...
<body>
	<!-- 
	국제화(formattion) 태그 : 국가별 다양한언어, 날짜, 시간, 숫자 형식을
		설정할때 사용한다. 접두어로는 fmt를 사용
 -->
	<h4>숫자 포맷 설정</h4>
	<!-- 변수 생성 -->
	<c:set var="number1" value="12345" />
	<!-- 
		groupingUsed : 세자리마다 ','를 출력. default값이 true이므로 ','를
			출력하고 싶지 않다면 false로 지정해야 한다.
	 -->
	콤마 O :
	...
	<!-- 
	 ...
	 -->
	<h4>문자열을 숫자로 변경</h4>
	<c:set var="number2" value="67,89.01" />
	<!-- 
		patern으로 주어진 부분을 분석하여 문장열을 숫자형식으로 파싱한다.
			해당 속성값으로
				000,000.00 사용시 정상
				0,0.0 사용시 정상
				,.사용시 에러발생
	 -->
	<fmt:parseNumber value="${ number2}" pattern="00,000.000" 
		var="printNum3" />
	소수점까지 : ${ printNum3 }
	<br />
