...
public class MemberAuth extends HttpServlet
{
	...
	/*
		클라가 최초요청시 서블릿객체가 생성되는데 이떄 최초1번만
		호출되는 init()메서드에서 DB연결 처리를 한다.
	*/
	@Override
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException
	{
		...
		*/
		...
		/*
			...
			아이디, 패스워드를 통해 회원인증을 진행한 후 일치하는 정보가 있따면,
			회원레코드를 DTO에 저장한 후 반환한다.
			만약 정보가 일치하지 않는다면 내용이없는 DTO를 반환한다.
		*/
		...
		{
			/*
				회원인증에 실패할 경우에는 서블릿 초기화 파라미터와 
				비교해서 최고관리자인지 재확인한다. 만약 최고관리자도
				아니라면 비회원이라 판단할 수 있따.
			*/
			if (admin_id.equals(id)) 
				req.setAttribute("authMessage", admin_id + "는 최고관리자이다.");
			else 
				req.setAttribute("authMessage", "귀하는 회원이 아니다");
		}
		// 처리할 메세지를 request영역에 저장한 후 JSP로 포워드 한다.
		req.getRequestDispatcher("/13Servlet/MemberAuth.jsp").forward(req, resp);
	}
	// 서블릿 종료시 DAO객체도 같이 자원 해제한다.
	@Override
	public void destroy() {
		dao.close();
	}
