...
/*
	어노에티션을 통한 요청명 매핑으로 *을 통해 여러 요청명을 한번에 매핑한다.
...
{
	/*
		get방식으로 들어오는 요청을 처리하기 위해 doGet메서드를 
		오버라이딩 해야 한다. 이 메서드가 정의되지 않았다면 405에러가 발생하게
		된다. 즉 해당 방식의 요청을 처리할 수 없다는 의미이다.
	*/
	...
	{	/*
		...
		// 앞에서 얻어온 인덱스를 통해 URL을 자른다. 즉, 마지막에 있는 요청명만
		// 남긴다.
		String commandStr= uri.substring(lastSlash);
		// 마지막 요청명을 통해 요청을 판단한 후 해당 요청을 처리할
		// 메서드를 호출한다. 이때 사용자의 요청정보를 담은 requset객체를 
		// 인수로 전달한다. 즉, 모든 요청을 메서드로 전달한다.
